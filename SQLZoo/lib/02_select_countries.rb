...
def large_countries
  ...
  FROM countries
  WHERE population > 200000000)AS large_countries
  SQL
end
...
def population_in_millions
  ...
  # millions.
  execute(<<-SQL)
  SELECT name, population_in_millions
  FROM (
  SELECT name, continent, population / 1000000 as population_in_millions
  FROM countries
  WHERE continent = 'South America') AS south_america
  SQL
end
